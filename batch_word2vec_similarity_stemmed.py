...
# Birleştirilmiş sonuçlar listesi
all_results = []
...
for config in model_configs:
    ...
    model_file = f"word2vec_stemmed_{model_type}_win{window}_dim{vector_size}.model"
    ...
    # Terminalde ilk 5 sonucu göster
...
# Tüm sonuçları birleştir ve tek dosyaya yaz
if all_results:
    final_df = pd.concat(all_results, ignore_index=True)
    final_df.to_csv("merged_word2vec_results_stemmed.csv", index=False)
    print("\n✅ Tüm sonuçlar 'merged_word2vec_results_stemmed.csv' dosyasına kaydedildi.")
